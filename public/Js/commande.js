...
window.onload = () => {
    ...
    /********************************************************************/
    d.querySelectorAll('#nom, #prenom, #email, #tel, #description').forEach(field => {
    ...
    /***********************************************************************************/
    d.querySelector('#commande-btn').addEventListener('click', (e) => {

        // On selectionne le formulaire de commande par son id
        let commandeForm = d.querySelector('#commandeform');
        ...
        if (!formError1.length) {
            ...
            fetch('/', {
                ...
                if (data.usercreated === true) {
                    ...
                    commandeZone.innerHTML = `<h2>Madame, Monsieur ${nom}, votre commande a bien été envoyée !</h2>`;
                }
